<?xml version="1.0" encoding="UTF-8"?><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:mn="https://www.metanorma.org/ns/standoc" xmlns:mnx="https://www.metanorma.org/ns/xslt" xmlns:mathml="http://www.w3.org/1998/Math/MathML" xmlns:xalan="http://xml.apache.org/xalan" xmlns:fox="http://xmlgraphics.apache.org/fop/extensions" xmlns:pdf="http://xmlgraphics.apache.org/fop/extensions/pdf" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:java="http://xml.apache.org/xalan/java" xmlns:jeuclid="http://jeuclid.sf.net/ns/ext" xmlns:barcode="http://barcode4j.krysalis.org/ns" xmlns:redirect="http://xml.apache.org/xalan/redirect" exclude-result-prefixes="java" extension-element-prefixes="redirect" version="1.0">
	...
	<xsl:variable name="numbers_japanese_">
		<xsl:copy-of select="/*/mn:localized-strings/mn:localized-string[starts-with(@key,'0') or starts-with(@key,'1') or starts-with(@key,'2') or starts-with(@key,'3') or     starts-with(@key,'4') or starts-with(@key,'5') or starts-with(@key,'6') or starts-with(@key,'7') or starts-with(@key,'8') or starts-with(@key,'9')]"/>
	</xsl:variable>
	...
	<xsl:template match="mn:bibdata[not(.//mn:passthrough)] |        mn:localized-strings" mode="update_xml_step1" priority="2">
		<xsl:choose>
			<xsl:when test="$vertical_layout = 'true'">
				<xsl:copy>
	...
	<xsl:template match="mn:p//text()[not(ancestor::mn:strong) and not(ancestor::mn:p[@class = 'zzSTDTitle2'])][normalize-space() != ''] |       mn:dt/text()[normalize-space() != ''] |        mn:biblio-tag/text()[normalize-space() != ''] |       item/title/text()" mode="update_xml_step1">
		<xsl:choose>
			<xsl:when test="$vertical_layout = 'true'">
				<xsl:call-template name="enclose_text_in_vertical_tag"/>
			</xsl:when>
...
 </xsl:text> <!-- to add distance between dt and dd -->
	...
	<xsl:template match="mn:references">
		<xsl:if test="not(ancestor::mn:annex)">
			<xsl:choose>
				<xsl:when test="following-sibling::mn:references or preceding-sibling::mn:references"/>
				<xsl:otherwise>
					<fo:block break-after="page"/>
				</xsl:otherwise>
			</xsl:choose>
...
&lt;</xsl:text>
	...
	<!-- From:
		...
		To:
			...
			<p>...</p>
		-->
	<xsl:template match="mn:foreword |            mn:foreword//mn:clause |            mn:preface//mn:clause[not(@type = 'corrigenda') and not(@type = 'policy') and not(@type = 'related-refs')] |            mn:introduction |            mn:introduction//mn:clause |            mn:sections//mn:clause |             mn:annex |             mn:annex//mn:clause |             mn:references[not(@hidden = 'true')] |            mn:bibliography/mn:clause |             mn:colophon |             mn:colophon//mn:clause |             mn:sections//mn:terms |             mn:sections//mn:definitions |            mn:annex//mn:definitions" mode="linear_xml" name="clause_linear">
		...
		<xsl:copy>
			...
			<xsl:if test="self::mn:foreword or self::mn:introduction or    local-name(..) = 'preface' or local-name(..) = 'sections' or     (self::mn:references and parent::mn:bibliography) or    (self::mn:clause and parent::mn:bibliography) or    self::mn:annex or     parent::mn:annex or    parent::mn:colophon">
